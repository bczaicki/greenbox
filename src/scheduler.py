...
class Task:
    ...
    def __init__(
        self, name, callback, interval=None, at_time=None, enabled=True
    ):
        """
        ...
        logger.debug(
            f"Task '{name}' created with "
            f"interval={interval}, at_time={at_time}"
        )
    ...
    def is_due(self):
        ...
        if self.at_time is not None:
            ...
            try:
                ...
                logger.error(
                    f"Invalid time format for task '{self.name}':"
                    f" {self.at_time}"
                )
                return False
